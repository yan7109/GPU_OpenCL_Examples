...
// oclAverageFilter is a multi-GPU enabled RGB Average filter
//*****************************************************************************
...
extern "C" double AverageFilterHost(unsigned int* uiInputImage, unsigned int* uiOutputImage, 
                                  unsigned int uiWidth, unsigned int uiHeight, int uiIter);

// Defines and globals for Average filter processing demo
//*****************************************************************************
#define REFRESH_DELAY	  10 //ms

int iBlockDimX = 30; //16
int iBlockDimY = 30; //4
int uiIter = 1;             // # of iterations 
int windowSize = 3; // must be odd number
int addedSize = windowSize/2 - 1 + uiIter;
// Global declarations
//*****************************************************************************
// Image data vars
const char* cImageFile = "butterflies.ppm";
...
// OpenCL vars
const char* clSourcefile = "AverageFilter.cl";  // OpenCL kernel source file
...
cl_kernel* ckAverage;                 // OpenCL Kernel array for Average
...
double AverageFilterGPU(unsigned int* uiInputImage, unsigned int* uiOutputImage);
...
int main(int argc, char** argv)
{
    ...
    shrSetLogFileName ("oclAverageFilter.txt");
    ...
    if((shrGetCmdLineArgumenti(argc, (const char**)argv, "device", &iSelectedDevice)) || (uiNumAllDevs == 1)) 
    {
        ...
        shrLog("  Using 1 Selected Device for Average Filter Computation...\n"); 
    ...
    else 
    {
        ...
        shrLog("    Using %u Device(s) for Average Filter Computation\n", GpuDevMngr->uiUsefulDevCt); 
    }
    ...
    ckAverage = new cl_kernel[GpuDevMngr->uiUsefulDevCt];
    ...
    {
        cqCommandQueue[i] = clCreateCommandQueue(cxGPUContext, GpuDevMngr->cdDevices[GpuDevMngr->uiUsefulDevs[i]], CL_QUEUE_PROFILING_ENABLE, &ciErrNum);
        oclCheckErrorEX(ciErrNum, CL_SUCCESS, pCleanup);
    ...
    if (ciErrNum != CL_SUCCESS)
    {
        ...
        oclLogPtx(cpProgram, oclGetFirstDev(cxGPUContext), "oclAverageFilter.ptx");
    ...
    for (cl_uint i = 0; i < GpuDevMngr->uiUsefulDevCt; i++)
    {
        ...
        ckAverage[i] = clCreateKernel(cpProgram, "ckAverage", &ciErrNum);
        oclCheckErrorEX(ciErrNum, CL_SUCCESS, pCleanup);
        shrLog("clCreateKernel (ckAverage), Device %u...\n", i); 
        ...
        cmDevBufIn[i] = clCreateBuffer(cxGPUContext, CL_MEM_READ_WRITE, szAllocDevBytes[i], NULL, &ciErrNum);
        oclCheckErrorEX(ciErrNum, CL_SUCCESS, pCleanup);
        cmDevBufOut[i] = clCreateBuffer(cxGPUContext, CL_MEM_READ_WRITE, szAllocDevBytes[i], NULL, &ciErrNum);
        oclCheckErrorEX(ciErrNum, CL_SUCCESS, pCleanup);
        ...
        int iLocalPixPitch = iBlockDimX + 2*addedSize;
        ciErrNum = clSetKernelArg(ckAverage[i], 0, sizeof(cl_mem), (void*)&cmDevBufIn[i]);
        ciErrNum |= clSetKernelArg(ckAverage[i], 1, sizeof(cl_mem), (void*)&cmDevBufOut[i]);
        ciErrNum |= clSetKernelArg(ckAverage[i], 2, (iLocalPixPitch * (iBlockDimY + 2*addedSize) * sizeof(cl_uchar4)), NULL);
        ciErrNum |= clSetKernelArg(ckAverage[i], 3, sizeof(cl_int), (void*)&iLocalPixPitch);
        ciErrNum |= clSetKernelArg(ckAverage[i], 4, sizeof(cl_uint), (void*)&uiImageWidth);
//        ciErrNum |= clSetKernelArg(ckAverage[i], 6, sizeof(cl_uint), (void*)&uiIter);        
//        ciErrNum |= clSetKernelArg(ckAverage[i], 7, sizeof(cl_uint), (void*)&windowSize);        
        oclCheckErrorEX(ciErrNum, CL_SUCCESS, pCleanup);
        shrLog("clSetKernelArg (0-4), Device %u...\n\n", i); 
    }

    // Set common global and local work sizes for Median kernel
    szLocalWorkSize[0] = iBlockDimX + 2*addedSize;
    szLocalWorkSize[1] = iBlockDimY + 2*addedSize;
    szGlobalWorkSize[0] = shrRoundUp(szLocalWorkSize[0], uiImageWidth + (int)(2*addedSize*uiImageWidth/iBlockDimX));
...
//*****************************************************************************
double AverageFilterGPU(cl_uint* uiInputImage, cl_uint* uiOutputImage)
{
    ...
    for (cl_uint i = 0; i < GpuDevMngr->uiUsefulDevCt; i++)
    {
        ...
        if (GpuDevMngr->uiUsefulDevCt == 1)
        {
            ...
            szGlobalWorkSize[1] = shrRoundUp((int)szLocalWorkSize[1], uiImageHeight + (int)(2*addedSize*uiImageHeight/iBlockDimY));
        ...
        ciErrNum |= clSetKernelArg(ckAverage[i], 5, sizeof(cl_uint), (void*)&uiDevImageHeight[i]);

        shrLog("The global work size is %i x %i\n", szGlobalWorkSize[0], szGlobalWorkSize[1]);
        shrLog("The local work size is %i x %i\n", szLocalWorkSize[0], szLocalWorkSize[1]);
        // Launch Average kernel(s) into queue(s) and push to device(s)
        ciErrNum |= clEnqueueNDRangeKernel(cqCommandQueue[i], ckAverage[i], 2, NULL, szGlobalWorkSize, szLocalWorkSize, 0, NULL, &event);

        shrLog("The TEST local work size is %i x %i, error number is %i\n", szLocalWorkSize[0], szLocalWorkSize[1], ciErrNum);
    ...
    cl_ulong start, end;
    cl_int status  = clGetEventProfilingInfo(event, CL_PROFILING_COMMAND_START, sizeof(cl_ulong), &start, NULL);
    status |= clGetEventProfilingInfo(event, CL_PROFILING_COMMAND_END, sizeof(cl_ulong), &end, NULL);
    if (status == CL_SUCCESS) {/*shrLog("Profiling Success!\n");*/}
    else shrLog("Profiling failed\n");

    double dKernelTime = (double)((end - start) * 1e-9f);
    int fps = (int)(1.0f/dKernelTime)*uiIter;
    shrLog("DEREK fps: %i\n\n", fps);
...
void InitGL(int* argc, char **argv)
{
    ...
    iGLUTWindowHandle = glutCreateWindow("OpenCL for GPU RGB Average Filter Demo");
#if !(defined (__APPLE__) || defined(MACOSX))
...
void DisplayGL()
{
    ...
    if (bFilter)
    {
        ...
        if (iProcFlag == 0)
        {
            dProcessingTime += AverageFilterGPU (uiInput, uiOutput);
        }
        else 
        {
            dProcessingTime += AverageFilterHost (uiInput, uiOutput, uiImageWidth, uiImageHeight, uiIter);
        }
    ...
    if (iFrameCount++ > iFrameTrigger)
    {
        ...
        iFramesPerSec = (int)((double)iFrameCount / dProcessingTime * uiIter);
        dProcessingTime /= (double)iFrameCount;
	if (iFramesPerSec < 0) iFramesPerSec = 0; 
        
#ifdef GPU_PROFILING
        if (bFilter)
        {
            #ifdef _WIN32
            sprintf_s(cTitle, 256, "%s RGB Average Filter ON | W: %u , H: %u | Iterations = %u | # GPUs = %u | ^%i^ fps | Proc. t = %.5f s | %.1f Mpix/s | window size = %i", 
                            cProcessor[iProcFlag], uiImageWidth, uiImageHeight, uiIter, GpuDevMngr->uiUsefulDevCt, iFramesPerSec, 
                            dProcessingTime, (1.0e-6 * uiImageWidth * uiImageHeight)/dProcessingTime, windowSize);  
            #else
                sprintf(cTitle, "%s RGB Average Filter ON | W: %u , H: %u | Iterations = %i | # GPUs = %u | %i fps | Proc. t = %.5f s | %.1f Mpix/s | window size = %i", 
                            cProcessor[iProcFlag], uiImageWidth, uiImageHeight, uiIter, GpuDevMngr->uiUsefulDevCt, iFramesPerSec, 
                            dProcessingTime, (1.0e-6 * uiImageWidth * uiImageHeight)/dProcessingTime, windowSize);  
            #endif
        }
        ...
        {
            #ifdef _WIN32
                sprintf_s(cTitle, 256, "RGB Average Filter OFF | W: %u , H: %u | %i fps", 
                            uiImageWidth, uiImageHeight, iFramesPerSec);  
            #else 
                sprintf(cTitle, "RGB Average Filter OFF | W: %u , H: %u | %i fps", 
                            uiImageWidth, uiImageHeight, iFramesPerSec);  
            #endif
        }
#else
        ...
        {
            #ifdef _WIN32
                sprintf_s(cTitle, 256, "%s RGB Average Filter ON | W: %u , H: %u | Iterations = %u", 
                            cProcessor[iProcFlag], uiImageWidth, uiImageHeight, uiIter);  
            #else
                sprintf(cTitle, "%s RGB Average Filter ON | W: %u , H: %u | Iterations = %u", 
                            cProcessor[iProcFlag], uiImageWidth, uiImageHeight, uiIter);  
            #endif
        }
        else 
        {
            #ifdef _WIN32
                sprintf_s(cTitle, 256, "RGB Average Filter OFF | W: %u , H: %u", 
                            uiImageWidth, uiImageHeight);  
            #else 
                sprintf(cTitle, "RGB Average Filter OFF | W: %u , H: %u", 
                            uiImageWidth, uiImageHeight);  
            #endif
        }
#endif
...
void TestNoGL()
{
    ...
    AverageFilterGPU (uiInput, uiOutput);

    // Start timer 0 and process n loops on the GPU
    const int iCycles = 150;
    dProcessingTime = 0.0;
    shrLog("\nRunning AverageFilterGPU for %d cycles...\n\n", iCycles);
    shrDeltaT(2); 
    ...
    shrLogEx(LOGBOTH | MASTER, 0, "oclAverageFilter, Throughput = %.4f M RGB Pixels/s, Time = %.5f s, Size = %u RGB Pixels, NumDevsUsed = %u, Workgroup = %u\n", 
    ...
    AverageFilterHost(uiInput, uiGolden, uiImageWidth, uiImageHeight, uiIter);
...
void Cleanup(int iExitCode)
{
    ...
    for (cl_uint i = 0; i < GpuDevMngr->uiUsefulDevCt; i++)
    {
        if(ckAverage[i])clReleaseKernel(ckAverage[i]);
